...
impl Workspace {
    ...
    /// The log pane in its inline form: fills whatever height is left, with an expand affordance
    /// that appears on hover.
    pub(super) fn render_log(&self, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            .w_full()
            .flex_1()
            .min_h(rems(6.))
    ...
    /// The log pane taking over the whole workspace area.
    pub(super) fn render_log_expanded(&self, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            .size_full()
            .overflow_hidden()
            .p_4()
