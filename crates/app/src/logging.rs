...
//!
//! This is the *developer* log — what the app itself did — and is separate from the run log the
//! user reads in the bottom dock, which is workbench's own stdout. Everything here shares one
//! thing: the path to the current session's log file, which is why the logger, the panic hook,
//! the log tail, and the dump that embeds it live in one module.
//!
//! The log rotates on startup rather than by size or date: `app.log` is the run you are in and
//! `app.old.log` is the one before it. That is the whole retention policy, and it is what makes
//! "reproduce it, then send me the log" work — the reproduction is always the current file.
//!
//! Nothing is uploaded. The dump travels by clipboard or by a prefilled GitHub issue URL, both
//! of which put the user in front of the text before it leaves the machine.
...
pub fn log_path() -> Option<PathBuf> {
    settings::data_dir().map(|dir| dir.join("logs").join("app.log"))
}
...
pub fn init() {
    let config = ConfigBuilder::new()
        ...
        // The HTTP stack narrates every request at debug level — headers, TLS handshake, pool
        // bookkeeping — which is dozens of lines per update check and per server ping, in a file
        // whose entire purpose is to be pasted into a bug report. Whoever made the call logs the
        // outcome itself (`update check failed: …`), which is the part anyone can act on.
    ...
    if let Some(path) = log_path() {
        if let Some(dir) = path.parent() {
            ...
            let _ = fs::rename(&path, dir.join("app.old.log"));
        }
    ...
    log::info!(
        "islandora-workbench-gui {} ({}) starting",
        version(),
        git_sha()
    );
}
...
/// frame, which is most of a session log. Neither is actionable from this app's log, so omit
/// those two notices while retaining other accessibility diagnostics.
///
/// Closing a window always produces the teardown errors: a detached per-window callback outlives
/// the window, reads it, and logs the expected miss. Kept in the file for teardown debugging, but
/// not at a level that makes every pasted bug report open with three errors nobody can act on.
...
pub fn debug_info(cx: &App, log_lines: usize) -> String {
    ...
    let _ = writeln!(out, "islandora-workbench-gui {} ({})", version(), git_sha());
    let _ = writeln!(
        out,
        "OS: {} {} [{}]",
    ...
    // The three settings that decide whether a run can even start. Nearly every report that is
    // not a UI bug is one of these pointing somewhere wrong, so they open the environment section.
    let _ = writeln!(out, "\nEnvironment:");
    for (label, key) in [
        ...
        let value = AppSettings::get(cx)
            .values
            .get(key)
            .map(|v| v.text().to_string())
            .filter(|v| !v.trim().is_empty());
        match value {
            // Whether the path is still there separates "never configured" from "moved or
            // uninstalled since", which are different bugs with the same symptom.
            Some(path) => {
                let exists = if Path::new(path.trim()).exists() {
                    "ok"
                } else {
                    "MISSING"
                };
                let _ = writeln!(out, "  {label}: {path}  [{exists}]");
            }
            None => {
                let _ = writeln!(out, "  {label}: (unset)");
            }
        }
    }
...
///
/// The dump carries the configured paths, which on a real machine means a real name — and the
/// point of a paste-it-yourself report is that what the user sees is what they send.
...
mod tests {
    ...
    fn redacts_the_home_directory() {
        ...
        let redacted = super::redact(&format!("workbench: {home}/islandora_workbench"));
    ...
    fn urlencodes_what_a_url_cannot_carry() {
        ...
        assert_eq!(super::urlencode("gui-0.1_x.y~z"), "gui-0.1_x.y~z");
    }
    ...
    // ponytail: no test that `debug_info` carries every section — it takes `&App`, and the only
    // way to build one in a test is gpui's `test-support` feature, which nothing else in this
    // workspace needs yet. Add that dev-dependency and qrate's `the_dump_carries_every_section`
    // if the dump grows conditional sections that could silently go missing.
}
